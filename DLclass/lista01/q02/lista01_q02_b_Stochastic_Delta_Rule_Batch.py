...
# -----------------------------------
def training_net_delta_batch(net_arc, net_func, w, b, data_in, target, learn_rate,
                 num_epochs, batchsize=0, err_max=0.0001):
    """
    ...
    oldw = np.copy(w)
    # Checking Batch size
    if batchsize > num_examples:
        batchsize = num_examples
    else:
        if batchsize:
            pass
        else:
    ...
    for ep in range(num_epochs):
        ...
        err_local = np.zeros(shape=(num_examples, 1))
        err_example_med = np.zeros(shape=(out_size, 1))
        ms_error = 0

        # shuffling the data set

        # for each example of the batch
        # Stochastic - shuffle in each epoch
        for example in list(shuffle(range(num_examples)))[0:batchsize]:
            ...
            # ---------------------
            # 2 - Error Measurement and accumulatino for each batch
            # ---------------------
            ...
            # mean of all errors on each presented example
            err_example_med += err_example

            err_local[example] = np.sum(err_example ** 2) / 2

        # ---------------------
        # 3 - Backpropagation for all examples on each batch
        # ---------------------
        err_example_med /= batchsize
        ...
        for layer in range(num_layers-1, 0, -1):
            ...
            if layer == (num_layers - 1):
                ...
                for neuron in range(out_size):
                    ...
                    gradi[layer, neuron] = err_example_med[neuron] * deriv_activation_func(net_func[layer], soma[layer, neuron])
        ...
        # Mean squarred error for this batchepoch
        ms_error = np.sum(err_local) / batchsize
...
# training model
(w1, epstop1, verro1) = training_net_delta(net_arc, net_func, w, b, ti, to, learn_rate, num_epochs)
(w2, epstop2, verro2) = training_net_delta_batch(net_arc, net_func, w, b, ti, to, learn_rate, 1000, 10)
(w3, epstop3, verro3) = training_net_delta_batch(net_arc, net_func, w, b, ti, to, learn_rate, num_epochs, 200)
...
plot_error(verro2[1:1000])

ypredok = [predict(forward(net_arc, net_func, w2, b, ti[x])) for x in range(50)]
...
xnovo
yteste
for i in list(shuffle(range(10)))[0:batchsize]:
    print(i)
