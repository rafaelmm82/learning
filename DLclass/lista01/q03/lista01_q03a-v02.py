...
data_pred_d = np.zeros(shape=(total_experiments, data_out.shape[0], 1))
...
for exp in range(total_experiments):
    ...
    data_pred_d[exp] = np.array([forward(net_arc, net_func, wd, b,
                                data_in[x]) for x in range(0, 4)])
    # Ploting data to graphicaly understanding
    plot_dataset_q03_a(data_in, data_pred_d[exp], data_out)


for i in range(9):
    plot_dataset_q03_a(data_in, data_pred_d[i], data_out)



# Ploting training and validation error history
plot_error_total_9(tetotal_delta, tetotal_delta)

# Ploting the means of training and validation error
plot_the_means_1(tetotal_delta)
